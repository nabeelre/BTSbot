...
def mm_cnn(config, image_shape, metadata_shape):
    ...
    x_conv = Conv2D(
        config['conv1_channels'], (config['conv_kernel'], config['conv_kernel']),
        activation='relu', padding='same', name='conv1', input_shape=image_shape
    )(triplet_input)
...
def mm_cnn_small(config, image_shape, metadata_shape):
    triplet_input = keras.Input(shape=image_shape, name="triplet")
    meta_input = keras.Input(shape=metadata_shape, name="metadata")
    ...
    model = keras.Model(inputs=[triplet_input, meta_input], outputs=output, name="mm_cnn_small")
    return model
