...
def run_val(output_dir, config):
    with open(output_dir+"report.json", 'r') as f:
        report = json.load(f)
    model_dir = output_dir + "model/"

    val_cuts_str = create_cuts_str(0, bool(config['val_sne_only']),
    ...
    return {"roc_auc": roc_auc, "bal_acc": bal_acc,
            "bts_acc": bts_acc, "notbts_acc": notbts_acc, 
            "precision": overall_precision, "recall": overall_recall, 
            "metric_precision": metric_precision, "metric_recall": metric_recall, 
