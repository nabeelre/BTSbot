...
def run_val(output_dir, config_path):
    ...
    if not (os.path.exists("data/triplets_v4_val_reg.npy") and 
            os.path.exists("data/candidates_v4_val_reg.csv")):
        
        create_validation_data(["trues", "dims", "vars", "MS"],  ztfids_val, for_regression=True)
    else:
        print("Validation data already present")

    cand = pd.read_csv("data/candidates_v4_val_reg.csv")

    print(f'num_notbts: {np.sum(cand.label == 0)}')
    print(f'num_bts: {np.sum(cand.label == 1)}')

    triplets = np.load("data/triplets_v4_val_reg.npy", mmap_mode='r')
    ...
    preds = model.predict([triplets, cand.loc[:,metadata_cols[:-2]]], batch_size=16, verbose=1)

    peakmag_preds = preds[0]
    raw_class_preds = preds[1]
    class_preds = np.rint(np.transpose(raw_class_preds))[0]
    
    peakmag_labels = cand["peakmag"].to_numpy()
    class_labels = cand["label"].to_numpy()

    cand["peakmag_preds"] = peakmag_preds
    cand["class_preds"] = class_preds

    mse = tf.keras.losses.MeanSquaredError(reduction="sum_over_batch_size")
    MSE_loss = mse(np.transpose([peakmag_labels]), peakmag_preds).numpy()
    print("Regression MSE Loss = " + str(MSE_loss))

    bce = tf.keras.losses.BinaryCrossentropy(from_logits=False)
    BCE_loss = bce(np.transpose([class_labels]), np.transpose([class_preds])).numpy()
    print("Classification BCE Loss = " + str(BCE_loss))

    bins = np.arange(15,21.5,0.5)
    narrow_bins = np.arange(17,21.00,0.25)
    
    TP_mask = np.bitwise_and(class_labels.astype(int), class_preds.astype(int))
    TN_mask = 1-(np.bitwise_or(class_labels.astype(int), class_preds.astype(int)))
    FP_mask = np.bitwise_and(1-class_labels.astype(int), class_preds.astype(int))
    FN_mask = np.bitwise_and(class_labels.astype(int), 1-class_preds.astype(int))
    ...
    fpr, tpr, thresholds = roc_curve(class_labels, raw_class_preds)
    roc_auc = auc(fpr, tpr)
    ...
    # MSE
    ax1.plot(report["Training history"]["peakmag_pred_loss"][::10], label='Training', linewidth=2)
    ax1.plot(report['Training history']['val_peakmag_pred_loss'][::10], label='Validation', linewidth=2)
    ax1.set_xlabel('Epoch')
    ax1.set_ylabel('MSE')
    ax1.legend(loc='best')
    ax1.set_ylim(np.percentile(report['Training history']['peakmag_pred_loss'], [0,90]))
    ax1.grid(True, linewidth=.3)

    # /===================================================================/

    plt.suptitle(model_dir[:-7], size=28)
    ax2.plot(report["Training history"]["class_pred_loss"][::10], label='Training', linewidth=2)
    ax2.plot(report['Training history']['val_class_pred_loss'][::10], label='Validation', linewidth=2)
    ax2.set_xlabel('Epoch')
    ax2.set_ylabel('BCE')
    ax2.legend(loc='best')
    ax2.set_ylim(np.percentile(report['Training history']['class_pred_loss'], [0,99]))
    ...
    ConfusionMatrixDisplay.from_predictions(class_labels, class_preds, normalize='true', 
    ...
    hist, xbins, ybins, im = ax7.hist2d(cand['magpsf'], raw_class_preds[:,0], norm=LogNorm(), bins=14, range=[[15, 22], [0, 1]])
    ax7.set_xlabel('magpsf')
    ...
    hist, xbins, ybins, im = ax8.hist2d(cand['magpsf'], peakmag_preds[:,0], norm=LogNorm(), bins=32, range=[[15, 22], [15, 22]])
    ax8.plot([15,22], [15,22], c='k')
    ax8.set_xlabel('magpsf')
    ax8.set_ylabel('Pred peakmag')
    ...
    hist, xbins, ybins, im = ax9.hist2d(cand['peakmag'], peakmag_preds[:,0], norm=LogNorm(), bins=32, range=[[15, 22], [15, 22]])
    ax9.plot([15,22], [15,22], c='k')
    ax9.set_xlabel('peakmag')
...
if __name__ == "__main__":
    run_val(sys.argv[1], "train_config_comb.json")